  ...
  return (
    ...
    >
      ...
      <GridItem area={"d"} className={styles.statsBox}>
       {<GraphStats title="Category Scores" desc="Survey Scores for Each Category" stats={''}
      graph={<CategoryChart title='' value={category}/>}/>}
      </GridItem>
      <GridItem area={"e"} className={styles.statsBox}>
      {<GraphStats title="Latest Violation" desc="Policy Violations by Date" stats={policies[0].date.toDateString().slice(4)}
